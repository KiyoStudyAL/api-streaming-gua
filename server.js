...
app.get('/api/watch', async (req, res) => {
    try {
        ...
        const selectedSource = sources.find(s => s.quality === '720') ||
                               sources.find(s => s.quality === '1080') ||
                               sources.find(s => s.quality === '480') ||
                               sources[0];
    ...
    <head>
        ...
        <style>
            *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
            :root {
                --red: #e50914; --bg: #0d0d0f; --surface: #18181c;
                --surface2: #222228; --border: rgba(255,255,255,0.07);
                --text: #f0f0f0; --muted: #888; --radius: 10px;
            }
            ...
            .logo { font-family: 'Rajdhani', sans-serif; font-size: 26px; font-weight: 700; color: var(--red); letter-spacing: 1px; }
            .logo span { color: var(--text); }
            .container { max-width: 860px; margin: 0 auto; padding: 24px 16px; }
            #player-container {
                display: none; background: #000; border-radius: var(--radius);
            ...
            #kualitas { padding: 10px 16px; background: var(--surface); display: flex; align-items: center; gap: 8px; font-size: 13px; color: var(--muted); flex-wrap: wrap; }
            #kualitas-btn button {
                background: var(--surface2); color: var(--text); border: 1px solid var(--border);
                padding: 4px 12px; border-radius: 5px; cursor: pointer; font-size: 12px;
                margin-left: 4px; transition: background 0.2s;
            }
            #kualitas-btn button:hover, #kualitas-btn button.aktif { background: var(--red); border-color: var(--red); }
            #judul-tayang {
            ...
            button.btn-cari {
                ...
                font-family: 'Inter', sans-serif; transition: opacity 0.2s; white-space: nowrap;
            }
            button.btn-cari:hover { opacity: 0.85; }
            #loading { display: none; text-align: center; color: var(--muted); margin: 32px 0; font-size: 14px; }
            .hasil-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 14px; }
            .anime-card {
            ...
            .anime-card .card-title { padding: 8px; font-size: 12px; line-height: 1.4; }
            .eps-grid { display: flex; flex-wrap: wrap; gap: 8px; }
            .eps-btn {
                background: var(--surface); border: 1px solid var(--border);
                padding: 8px 14px; border-radius: 7px; cursor: pointer;
                font-size: 13px; transition: background 0.2s; color: var(--text);
            }
            .eps-btn:hover { background: var(--surface2); }
            .eps-btn.active { background: var(--red); border-color: var(--red); font-weight: 600; }
            @media (max-width: 480px) { .hasil-grid { grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); } }
        </style>
    </head>
    <body>
        <header><div class="logo">ANIME<span>KU</span></div></header>
        <div class="container">
            ...
            <div class="search-box">
                <input type="text" id="input-cari" placeholder="Cari anime... (misal: Jujutsu Kaisen)" onkeydown="if(event.key==='Enter') cariAnime()">
                ...
                try {
                    ...
                    if (data.sukses && data.data.length > 0) {
                        data.data.forEach(anime => {
                            const title = typeof anime.title === 'string' ? anime.title : (anime.title?.english || anime.title?.romaji || "Tanpa Judul");
            ...
            async function nontonEps(eps_id, btnElement) {
                ...
                try {
                    ...
                    if (data.sukses) {
                        ...
                        btnBox.innerHTML = '';
                        if (data.semua && data.semua.length > 1) {
                            data.semua.forEach((src) => {
                                const b = document.createElement('button');
                                b.innerText = src.quality || 'Auto';
                                if (src.url === data.link) b.classList.add('aktif');
                                b.onclick = () => {
                                    document.querySelectorAll('#kualitas-btn button').forEach(x => x.classList.remove('aktif'));
